...
cache_data = pickle.load(open('/mnt/nas-data-1/zhanglingjun.zlj1/data/nuscene/data/cached_nuscenes_info.pkl', 'rb'))
...
def planning_evaluation(pred_trajs_dict, config):
    ...
    # for index, token in enumerate(tqdm(pred_trajs_dict.keys())):
        ...
        try:
            ...
            # data = re.findall(r'\((\-?\d+\.?\d*),\s*(\-?\d+\.?\d*)\)', pred_trajs_dict[token])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gt_folder', type=str, default='/mnt/nas-data-1/zhanglingjun.zlj1/data/nuscene/data/metrics')
