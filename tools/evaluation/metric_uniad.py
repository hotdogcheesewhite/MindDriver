...
class PlanningMetric(Metric):
            ...
            for ts in collision_time_steps:
                if 0 in segmentation[ts].cpu().numpy():
                    if flag:
                        ...
                        # with open('collision_tokens.txt', 'a') as f:
                        #     f.write(f'{token}\n')
                        flag = False
    ...
    def evaluate_coll(self, trajs, gt_trajs, segmentation, token=None):
        ...
        for i in range(B):
            ...
            coll_mask = segmentation[i, ti[m1], yi[m1], xi[m1]].long()
            obj_coll_sum[ti[m1]] += coll_mask
            # if coll_mask.sum() > 0:
            #     with open('/mnt/nas-data-1/zhanglingjun.zlj1/FSDrive-main/collision_tokens.txt', 'a') as f:
            #         f.write(f'{token}\n')
            ...
            # if box_coll[m2].any():   # 
            #     with open('/mnt/nas-data-1/zhanglingjun.zlj1/ad_data_process/eval/v4_collision_tokens_13000.txt', 'a') as f:
            #         f.write(f'"{token}",\n')
    ...
    def update(self, trajs, gt_trajs, segmentation, token=None, gt_trajs_mask=None):
        ...
        mask_list = gt_trajs_mask.cpu().numpy().astype(bool).tolist()

        obj_coll_sum, obj_box_coll_sum = self.evaluate_coll(trajs[:,:,:2], gt_trajs[:,:,:2], segmentation, token=token)
        obj_box_coll_sum_list = obj_box_coll_sum.cpu().numpy().astype(int).tolist()
